...
namespace RDFSharp.Semantics.Extensions.TIME.Test
{
    ...
    public class TIMEInstantPositionTest
    {
        ...
        public void ShouldThrowExceptionOnCreatingNumericInstantPositionBecauseNullTRS()
            => Assert.ThrowsException<OWLSemanticsException>(() => new TIMEInstantPosition(new RDFResource("ex:instPos"), null, 121977842));

        [TestMethod]
        public void ShouldThrowExceptionOnCreatingNominalInstantPositionBecauseNullTRS()
            => Assert.ThrowsException<OWLSemanticsException>(() => new TIMEInstantPosition(new RDFResource("ex:instPos"), null, new RDFResource("ex:Archean")));

        [TestMethod]
        public void ShouldThrowExceptionOnCreatingNominalInstantPositionBecauseNullValue()
            => Assert.ThrowsException<OWLSemanticsException>(() => new TIMEInstantPosition(new RDFResource("ex:instPos"), TIMEPositionReferenceSystem.GeologicTRS, null));
        #endregion
    }
}