...
namespace RDFSharp.Semantics.Extensions.TIME
{
    ...
    public class TIMECalendarReferenceSystem : TIMEReferenceSystem
    {
        ...
        {
            if (trsMetrics == null)
                throw new OWLSemanticsException("Cannot create calendar-based TRS because given \"trsMetrics\" parameter is null");
    ...
    public class TIMECalendarReferenceSystemMetrics
    {
        ...
        public TIMECalendarReferenceSystemMetrics(uint secondsInMinute, uint minutesInHour, uint hoursInDay, uint[] months)
        {
            ...
            if (secondsInMinute == 0)
                throw new OWLSemanticsException("Cannot build calendar metrics because given \"secondsInMinute\" parameter must be greater than zero");
            if (minutesInHour == 0)
                throw new OWLSemanticsException("Cannot build calendar metrics because given \"minutesInHour\" parameter must be greater than zero");
            if (hoursInDay == 0)
                throw new OWLSemanticsException("Cannot build calendar metrics because given \"hoursInDay\" parameter must be greater than zero");
            if (months == null)
                throw new OWLSemanticsException("Cannot build calendar metrics because given \"months\" parameter is null");
            if (months.Length == 0)
                throw new OWLSemanticsException("Cannot build calendar metrics because given \"months\" parameter must contain at least one element");
            if (months.Contains<uint>(0))
                throw new OWLSemanticsException("Cannot build calendar metrics because given \"months\" parameter must contain all elements greater than zerp");
            #endregion
